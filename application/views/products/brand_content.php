                        ...
                        <?php if (!empty($data)) : ?>
                            <?php foreach ($data as $row) : ?>
                                <tr>
                                    ...
                                    <td><?= date('Y-m-d', strtotime($row->created_at)) ?? 'Null' ?></td>
                                    <td> <span class="badge bg-label-<?= $row->is_active ? "success" : "danger" ?>"><?= $row->is_active ? "Active" : "InActive" ?></span></td>
                                    <td>
                                        <a data-id="<?= $row->id ?>" title="View Brand" href="javascript:void(0)" class="view_brand badge bg-label-primary"><span class="bx bx-detail"></span></a>
                                        <a data-id="<?= $row->id ?>" title="Edit Brand" href="javascript:void(0)" class="edit_brand badge bg-label-info"><span class="bx bxs-pencil"></span></a>
                                        <a data-id="<?= $row->id ?>" title="Delete Brand" href="javascript:void(0)" class="delete_brand badge bg-label-danger"><span class="bx bxs-trash-alt"></span></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else : ?>
